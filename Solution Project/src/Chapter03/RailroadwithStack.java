...
public class RailroadwithStack {
	...
	static void railroad(int[] cars, int nTracks) {
		int n = cars.length;
		Stack<Integer>[] tracks = new Stack[nTracks];
			...
			for (int i = 0; i < n; i++) {
				...
				if (current == expected) {
					...
					while (smallestInTrack(tracks) == expected) {
						popCar(tracks, expected);
						expected++;
					}
				} else {
					pushCar(tracks, current);
				}
			}
	...
	// 대기선로에 차량을 추가 
	static void pushCar(Stack<Integer>[] tracks, int car) throws UnableOpException {
		int target = -1;
		for (int i = 0; i < tracks.length; i++) {
			if (tracks[i].empty()) {
				if (target == -1) {
	...
	// 대기선로에서 해당 차량을 꺼내옴 
	static void popCar(Stack<Integer>[] tracks, int car) throws UnableOpException {
		for (int i = 0; i < tracks.length; i++) {
			if (tracks[i].empty()) {
				continue;
			}
	...
	static int smallestInTrack(Stack<Integer>[] tracks) {
		int smallest = Integer.MAX_VALUE;

		for (int i = 0; i < tracks.length; i++) {
			if (!tracks[i].empty() && tracks[i].peek() < smallest) {
